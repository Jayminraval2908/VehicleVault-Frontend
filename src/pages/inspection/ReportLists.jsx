...
const ReportsList = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        const data = await inspectionService.getReportsByVehicle(vehicleId);
  ...
  return (
    <div className="p-8 bg-[#0D0D0D] min-h-screen text-white">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-gray-500 mb-10">Detailed technical analysis and grading for Vehicle ID: #{vehicleId?.slice(-6)}</p>

        {reports.length === 0 ? (
          <div className="text-center py-24 bg-[#111111] border border-dashed border-gray-800 rounded-3xl">
          ...
          <div className="space-y-8">
            {reports.map((report) => (
              <Card key={report._id} className="relative overflow-hidden group border-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
                  ...
                  <div className="bg-[#0D0D0D] p-5 rounded-2xl border border-gray-800">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-800 text-[10px] text-gray-600 uppercase flex justify-between">
                      <span>Verified On: {new Date(report.createdAt).toLocaleDateString()}</span>
