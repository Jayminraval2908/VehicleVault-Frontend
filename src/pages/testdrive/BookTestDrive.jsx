...
const BookTestDrive = () => {
  ...
  const [formData, setFormData] = useState({
    date: "",
    time: "",
    location: "Main Showroom - Luxury Wing",
  });
  ...
  const handleBooking = async (e) => {
    ...
    if (!vehicleId) {
      toast.error("Invalid vehicle. Please try again.");
      return;
    }
    ...
    try {
      ...
      await testDriveService.bookTestDrive(dataToSend);
      ...
      toast.error(
        err.response?.data?.message || "Scheduling failed. Please try again."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#0D0D0D] p-6 flex justify-center items-center">
      <Card className="max-w-xl w-full border-[#D4AF37]/10 shadow-2xl p-10">
        ...
        <form onSubmit={handleBooking} className="space-y-6">
          ...
          <div className="bg-[#1a1a1a] p-4 rounded-xl border-l-4 border-[#D4AF37]">
            <p className="text-xs text-gray-400 leading-relaxed">
              * Please ensure you carry a valid driver's license at the time of
              the test drive. Confirmations are sent via email within 24 hours.
            </p>
          </div>
          ...
          <Button
            type="submit"
            className="w-full py-4 text-md font-bold shadow-gold"
          >
            CONFIRM BOOKING
          </Button>
        </form>
